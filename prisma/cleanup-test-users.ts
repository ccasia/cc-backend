...
async function cleanupTestUsers(batchNumber: number, dryRun = false) {
  ...
  for (let i = 0; i < userIds.length; i += CHUNK_SIZE) {
    ...
    try {
      await prisma.$transaction(async (tx) => {
        console.log(
          `🗑️  Processing chunk ${Math.floor(i / CHUNK_SIZE) + 1}/${Math.ceil(userIds.length / CHUNK_SIZE)}...`,
        );
        ...
        if (submissions.length > 0) {
          ...
          await tx.submissionDependency.deleteMany({
            where: {
              OR: [{ submissionId: { in: submissionIds } }, { dependentSubmissionId: { in: submissionIds } }],
            },
          });
